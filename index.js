...
function startVideoRotation() {
  ...
  videoRotationInterval = setInterval(() => {
    if (isStreaming) {
      console.log('🔄 Rotating to next video...');
      rotateToNextVideo();
    }
...
function rotateToNextVideo() {
  ...
  console.log('🎥 Switching to video ' + (currentVideoIndex + 1) + '/' + VIDEO_URLS.length);
...
function startStream() {
  ...
  try {
    ...
    console.log('Using video: ' + (currentVideoIndex + 1) + '/' + VIDEO_URLS.length);
...
app.get('/status', (req, res) => {
  res.json({
    ...
    currentVideoUrl: VIDEO_URLS[currentVideoIndex],
...
app.get('/', (req, res) => {
  ...
  const html = '<html><head><title>Pepe Livestreamer - Auto Mode</title>' +
    ...
    '<p><strong>Current Video:</strong> <span id="video">' + videoNum + '/' + VIDEO_URLS.length + '</span></p>' +
    ...
    'document.getElementById("video").textContent=data.currentVideo+"/"+data.totalVideos;' +
